...
class Bonus extends \Gazelle\BaseUser {
    ...
    public function hourlyRate(): float {
        return (float)self::$db->scalar("
            SELECT sum(bonus_accrual(e.Size, xfh.seedtime, tls.Seeders))
            FROM (
            ...
            INNER JOIN edition AS e ON (e.edition_id = xfu.fid)
            INNER JOIN torrents_leech_stats tls ON (tls.TorrentID = e.edition_id)
            WHERE xfu.uid = ?
            ", $this->id(), $this->id()
        );
    }
    ...
    public function userTotals(): array {
        $stats = self::$db->rowAssoc("
            SELECT count(*) AS total_torrents,
                ...
                coalesce(sum(bonus_accrual(e.Size, xfh.seedtime,                           tls.Seeders)), 0)                           AS hourly_points,
                coalesce(sum(bonus_accrual(e.Size, xfh.seedtime + (24 * 1),                tls.Seeders)), 0) * (24 * 1)                AS daily_points,
                coalesce(sum(bonus_accrual(e.Size, xfh.seedtime + (24 * 7),                tls.Seeders)), 0) * (24 * 7)                AS weekly_points,
                coalesce(sum(bonus_accrual(e.Size, xfh.seedtime + (24 * 365.256363004/12), tls.Seeders)), 0) * (24 * 365.256363004/12) AS monthly_points,
                coalesce(sum(bonus_accrual(e.Size, xfh.seedtime + (24 * 365.256363004),    tls.Seeders)), 0) * (24 * 365.256363004)    AS yearly_points,
                if (coalesce(sum(e.Size), 0) = 0,
                    0,
                    sum(bonus_accrual(e.Size, xfh.seedtime + (24 * 365.256363004),    tls.Seeders)) * (24 * 365.256363004)
                    / (sum(e.Size) / (1024*1024*1024))
                ) AS points_per_gb
            FROM (
                SELECT DISTINCT uid, fid
                FROM xbt_files_users
    ...
    public function seedList(string $orderBy, string $orderWay, int $limit, int $offset): array {
        self::$db->prepared_query("
            SELECT
                ...
                bonus_accrual(e.Size, xfh.seedtime,                           tls.Seeders)                           AS hourly_points,
                bonus_accrual(e.Size, xfh.seedtime + (24 * 1),                tls.Seeders) * (24 * 1)                AS daily_points,
                bonus_accrual(e.Size, xfh.seedtime + (24 * 7),                tls.Seeders) * (24 * 7)                AS weekly_points,
                bonus_accrual(e.Size, xfh.seedtime + (24 * 365.256363004/12), tls.Seeders) * (24 * 365.256363004/12) AS monthly_points,
                bonus_accrual(e.Size, xfh.seedtime + (24 * 365.256363004),    tls.Seeders) * (24 * 365.256363004)    AS yearly_points,
                bonus_accrual(e.Size, xfh.seedtime + (24 * 365.256363004),    tls.Seeders) * (24 * 365.256363004)
                    / (e.Size / (1024*1024*1024)) AS points_per_gb
            FROM (
            ...
            INNER JOIN xbt_files_history AS xfh USING (uid, fid)
            INNER JOIN edition AS e ON (e.edition_id = xfu.fid)
            INNER JOIN torrents_leech_stats AS tls ON (tls.TorrentID = e.edition_id)
            WHERE
                xfu.uid = ?
        ...
        foreach (self::$db->to_array('ID', MYSQLI_ASSOC, false) as $r) {
            $r['torrent'] = new \Gazelle\Edition($r['ID']);
            $list[] = $r;
        }
        return $list;
    }
